...
class Invaders(GameApp):
    ...
    def _STATE_CONTINUE(self):
        """
        Helper for update for when in continue state.
        """
        if self._timer > 0:
            self._text = GLabel(text = '\'We lost a fighter!\' \n \'Keep going! \n Those TIEs won\'t last forever!\'', \
                                x = GAME_WIDTH/2, y = GAME_HEIGHT/2, font_size = 45, font_name = 'Arcade', \
                                bold = True, halign = 'center', valign = 'middle', linecolor = 'red')
        elif self._timer == 0:
            self._wave.set_Ship()
            self._text = None
